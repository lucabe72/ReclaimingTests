...
void run(int ind, ...)
{
	...
	//t_totexec = *min;
	t_totexec = msec_to_timespec(max_run(timespec_to_msec(min), timespec_to_msec(max)));
...
void *thread_body(void *arg)
{
	...
	switch (data->sched_policy)
	{
		...
		case deadline:
			...
			if (data->sched_flags && SCHED_FLAG_RECLAIM)
				fprintf(data->log_handler, "GRUB Reclaiming!");
			fprintf(data->log_handler, "\n");
			attr.sched_policy = SCHED_DEADLINE;
			attr.sched_priority = 0;
			attr.sched_runtime = timespec_to_nsec(&data->budget);
//			attr.sched_runtime  = timespec_to_nsec(&data->period);
