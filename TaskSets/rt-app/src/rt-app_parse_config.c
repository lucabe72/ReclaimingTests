...
{
	long bcet, wcet, period, period_jitter, dline, runtime;
	char *policy;
	...
	/* exec time */
	runtime = get_int_value_from(obj, "runtime", FALSE, 0);
	if (runtime > period) {
	...
	wcet = get_int_value_from(obj, "wcet", TRUE, runtime);
	bcet = get_int_value_from(obj, "bcet", TRUE, runtime - 1000);
	data->min_et = usec_to_timespec(bcet);
	data->max_et = usec_to_timespec(wcet);
	data->budget = usec_to_timespec(runtime);
	...
	if (get_bool_value_from(obj, "reclaiming", TRUE, 0))
		data->sched_flags |= SCHED_FLAG_RECLAIM;
...
parse_tasks(struct json_object *tasks, rtapp_options_t *opts)
{
	...
	foreach (tasks, entry, key, val, idx) {
		if (parse_thread_data(key, val, idx, &opts->threads_data[idx], opts) < 0) {
			idx--;
			opts->nthreads--;
		}
	}
