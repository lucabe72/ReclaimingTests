...
{
  return n(ts, j, i) * ts->t[j].c + min(ts->t[j].c, ts->t[i].d - n(ts, j, i) * ts->t[j].p);
}
...
/*
	...
	and 
       ...
       I_j =  n_j C_j + min(C_j, D_i - n_j T_j) 

      and  

       n_j = \floor{\frac{D_i}{T_j}}
 */
unsigned int is_schedulable_baker(const struct taskset *ts, unsigned int m)
{
  unsigned int i;
